...
`--raw-visibility` の出力ディレクトリは `frinZ/rawvis/`、ファイル名は `_rawvis` 接尾辞です。raw visibilityは `.cor` 読み込み直後のrow-major `[sector,channel]` で、ACF規格化、rebin、padding、bandpass補正より前の値です。`--delay` または `--rate`（両方可）を指定すると、元データ図に加えて同じ補正を全時間・全周波数セルへ適用した `*_rawvis_corrected_delay_heatmap_{amp,phase}.png`、`*_rawvis_corrected_rate_heatmap_{amp,phase}.png`、または `*_rawvis_corrected_delay_rate_heatmap_{amp,phase}.png` を出力します。
